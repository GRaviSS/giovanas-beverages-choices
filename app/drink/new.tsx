...
/**
 * Tela de formulário para adicionar ou editar drink
 * Funciona tanto para criar novo quanto para editar existente
 */
export default function DrinkFormScreen() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id?: string }>();
  const { drinks, addNewDrink, updateExistingDrink } = useDrinks();
  const isEditing = !!id;
  
  // Encontra o drink se estiver editando
  ...
  // Carrega o drink se estiver editando
  useEffect(() => {
    if (isEditing && existingDrink) {
      console.log('[DrinkFormScreen] Carregando drink para edição:', existingDrink.id);
      form.loadDrink(existingDrink);
    }
  }, [isEditing, existingDrink]);
  ...
  const handlePickImage = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        console.log('[DrinkFormScreen] Foto selecionada');
        form.setPhoto(result.assets[0].uri);
      }
    } catch (error) {
      console.error('[DrinkFormScreen] Erro ao selecionar foto:', error);
  ...
  const handleRemovePhoto = () => {
    Alert.alert('Remover foto', 'Deseja remover a foto?', [
      { text: 'Cancelar', style: 'cancel' },
      {
        ...
        style: 'destructive',
        onPress: () => {
          console.log('[DrinkFormScreen] Removendo foto');
  ...
  const handleSave = async () => {
    ...
    try {
      const drinkData = form.getDrinkData(isEditing ? id : undefined);
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.form}>
        ...
        <View style={styles.actions}>
          ...
          />
          <Button
            title={isEditing ? 'Salvar alterações' : 'Salvar drink'}
            onPress={handleSave}
            variant="primary"
            style={styles.actionButton}
          />
